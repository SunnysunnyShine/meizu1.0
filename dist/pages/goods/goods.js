  ...
  require(["jquery", "sw"], function ($, Swiper) {
    ...
    $.ajax({
      ...
      success: function success(msg) {
        ...
        $(".nav-list")[0].innerHTML = template("nav-top-list", nav_data); //导航列表项 a标签
        //划入显示的商品列表	
        ...
        for (var j = 0; j < 4; j++) {
          ...
          nav_hover[j].onmouseover = function () {
            ...
            $(".nav-hover-list").show();
            $("#_bac").removeClass("bac").addClass("b_hover"); //对应生成列表
    ...
    $.ajax({
      ...
      success: function success(msg) {
        ...
        console.log(msg);
        var goodsList = {};
        var goodsArr = []; // for (var i = 0; i < msg.block_363.length; i++) {
        ...
        goodsArr.push(goodsList); // goodsList = {};	
        // }

        console.log(goodsArr);
        $(".container").html(template("goodMsg", goodsArr)); // 立即购买绑定事假
        ...
        $(".property-buy-action").find("a").click(function () {
          ...
          if (str == null) {
            //如果cookie是空的,表示第一次保存商品信息
            var arr = []; //写下面，因为有判断,不是同一个元素才推进数组,否则就数量加一
            //将对象里面的东西放进去
            // arr.push(pl);
            //写下面，全部判断结束后才放入
            //将数组的东西存入cookie
            ...
            //cookie中已存在商品信息
            //取出数组里面的东西先放进数组在
            var arr = JSON.parse(str); //写下面，因为有判断,不是同一个元素才推进数组,否则就数量加一
            //将对象里面的东西放进去
            // arr.push(pl);
            //写下面，全部判断结束后才放入
            //再将数组的东西存入cookie
