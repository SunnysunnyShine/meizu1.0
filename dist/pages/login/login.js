...
require(["../../static/conf/config.js"], function () {
  require(["jquery", "sw"], function ($, Swiper) {
    ...
    function getPagePos(ele) {
      var sum = ele.offsetLeft; //获取到 body时offsetLeft返回值为0，查找body的父级返回为null ,循环结束
    ...
    var slic = document.getElementsByClassName("verify-slice")[0]; //鼠标点击验证按钮时，上方的拼图显示出来
    ...
    var start = 20; //鼠标划入滑块位置时 上方的拼图显示出来
    ...
    document.onmouseup = function () {
      ...
      if (Math.abs(slic.offsetLeft - 157) < 5) {
        $("#verify").addClass("visiblility-hidden"); //$("#verify").css('display','none');
